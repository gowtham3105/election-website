...
class NavBar extends Component {
	...
	async isAdmin(tokenId) {
		console.log(tokenId, 'tokenid', this.state.tokenId, 'state tokenid');
		if (this.state.tokenId.length || (tokenId && tokenId.length)) {
			const token = tokenId || this.state.tokenId;
			setInfo({
				isAdmin: this.state.isAdmin,
				isVoter: this.state.isVoter,
				isModerator: this.state.isModerator,
			...
			await fetch(api_endpoint + '/api/userType?tokenId=' + token)
				.then((response) => {
				...
				.then((user) => {
					if (user.type === 'adminANDvoter') {
						...
						Swal.fire({
							icon: 'success',
							title: 'Logged In Successfully',
							text: 'You are Logged in as Voter',
						});
					} else {
						this.setState({
							isAdmin: false,
              ...
              isModerator: false,
						...
						Swal.fire({
							icon: 'error',
							title: 'Oops...',
							text: 'Unable to Find Roles, Please Try Again',
						});
					}
				})
				.then(() => {
					...
					>
						...
						<Nav fill>
							<Nav.Link>
								<Login
									isSigned={this.state.isSigned}
									onLoginSuccess={this.signInOnSuccess}
									onLoginFailure={this.signInOnError}
									...
									onLogoutFailure={this.signOutOnError}
									handleOTPSignIn={this.handleOTPSignIn}
								/>
								{/* {!this.state.isSigned ? (
									<GoogleLogin
...
class Account extends Component {
	...
	showpos = () => {
		if (this.state.details.voter_rights.length)
			return (
				<ul>
					{this.state.details.voter_rights
						.sort(function (a, b) {
	...
	componentDidMount() {
		...
		fetch(api_endpoint + '/api/positions')
			...
			.then((positions) => {
				...
				//  console.log(total);
				this.setState({
					posapi: positions.sort(this.sortfun),
					loadContent: load,
					totcand: total,
				}); // NOTA subtract
			});
