        ...
        <Navbar collapseOnSelect expand="lg"  variant="light" className = "NavBar" >
          ...
          <Navbar.Collapse id="responsive-navbar-nav" className='NavBar' >
            <Nav className="mr-auto " fill>
              {firebase.auth().currentUser ? <><Nav.Link onClick={() => { setShowAccount(true);console.log("here")}} className="NavLink">Account</Nav.Link></>:""}
            ...
            <Nav fill> 
              <Nav.Link ><Button onClick={() => {if (!(this.props.loginState)){this.login()}else{this.logout()}}} className='Button'>{this.props.loginState ? 'Sign Out':'Sign In'}</Button></Nav.Link>
            </Nav>
  ...
  render() {
    return (
      <div>
        ...
        {(this.props.show && firebase.auth().currentUser )?<div>Account</div>:""}
      </div>
    )
  }
}
  ...
  render(){
    return(
      ...
      <Router>
        ...
        <Account show={this.state.showAccount} details={this.state.accountDetails}/>
      </Router>
